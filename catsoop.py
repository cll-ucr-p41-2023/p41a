...
__HTTP_PORT = baseObj.catsoop_ports[0]
__SOCKET_PORT = baseObj.catsoop_ports[1]

# Make it possible for glowcomm.html to find out what the websocket port is:
js = __file__.replace(
    'catsoop.py', 'vpython_libraries' + os.sep + 'glowcomm.html')
...
class serveHTTP(BaseHTTPRequestHandler):
    serverlib = __file__.replace('catsoop.py', 'vpython_libraries')
    serverdata = __file__.replace('catsoop.py', 'vpython_data')
