...
import atexit
# Python 3; needed to allow zero arguments in a bound function
...
# from . import __version__, __gs_version__ [UCR]
import sys
import colorsys
...
# version = [__version__, 'jupyter'] [UCR]
# GSversion = [__gs_version__, 'glowscript'] [UCR]
version = None  # [UCR]
GSversion = None  # [UCR]
    ...
    if sendval in __vecattrs:  # it would be good to do some kind of compression of doubles
        ...
        if sendval == 'data' and len(val) == 0:
            ...
            for p in val:
                s += "{:.16G},{:.16G},".format(p[0], p[1])
        s = s[:-1]
    # val is the vertex object referenced by triangle or quad
    elif sendval in ['v0', 'v1', 'v2', 'v3']:
        s += str(val.idx)
    elif sendval == 'vs':
        if len(val) == 3:
            s += "{d},{d},{d}".format(val[0].idx, val[1].idx, val[2].idx)
        else:
            s += "{d},{d},{d},{d}".format(val[0].idx,
                                          val[1].idx, val[2].idx, val[3].idx)
    elif ismethods:
...
class baseObj(object):
    visualize = True  # [UCR]
    catsoop = False  # [UCR] Flag for running sim on CAT-SOOP
    catsoop_ports = [-1, -1]  # [UCR] HTTP and SOCKET ports
    glow = None
    objCnt = 0
    ...
    # entries are [invisible object to follow, function to call for pos, prevous pos]
    follow_objects = []
        ...
        for aa in cls.attach_arrows:
            ...
            if not isinstance(vval, vector):
                raise AttributeError(
                    'attach_arrow attribute "'+aa.attr+'" value must be a vector.')
            if aa._last_pos.equals(obj._pos) and aa._last_val.equals(vval):
                continue
    ...
    def __init__(self, **kwargs):
        if not (baseObj._view_constructed or
            ...
            elif baseObj.visualize:  # [UCR]
                from .no_notebook import _
            elif baseObj.catsoop:  # [UCR]
                from .catsoop import _
    ...
    def appendcmd(self, cmd):
        ...
        if (self.visualize or self.catsoop):  # [UCR]
            cmd['idx'] = self.idx
            while not baseObj.sent:  # baseObj.sent is always True in the notebook case
                time.sleep(0.001)
            # this is an "atomic" (uninterruptable) operation
...
class GlowWidget(object):
    def __init__(self, wsport=None, wsuri=None):
        ...
        if _isnotebook:
            ...
            if (wsport):
                self.comm = Comm(target_name='glow', data={
                                 'wsport': wsport, 'wsuri': wsuri})
            else:
    ...
    def handle_msg(self, msg):
        # this is a list of time-ordered events
...
def _wait(cvs):  # wait for an event
    cvs._waitfor = None
    if _isnotebook:
        baseObj.trigger()  # in notebook environment must send methods immediately
...
class standardAttributes(baseObj):
    ...
    attrLists = {'box': [['pos', 'color', 'trail_color'],
                 ...
                 'curve': [['origin', 'color'],
                           ['axis', 'size', 'up'],
                           ['visible', 'shininess', 'emissive',
                            'radius', 'retain', 'pickable'],
                           ['red', 'green', 'blue', 'length', 'width', 'height']],
                 'points': [['color'],
                            [],
                            ['visible', 'shininess', 'emissive', 'radius',
                             'retain', 'pickable', 'size_units'],
                            ['red', 'green', 'blue']],
    ...
    def setup(self, args):
        ...
        if objName[:8] == 'compound':
            objName = 'compound'
        del args['_objName']

    # default values
        # axis/size connection is the default; False for sphere, ring, text, compound
        self._sizing = True
            ...
            if a in args:
                ...
                if objName == 'extrusion' and a == 'color':
                    ...
                    if isinstance(val, vector):
                        # '_' bypasses setters; copy of val
                        setattr(self, '_'+a, vector(val))
                    else:
    ...
    # so that setting axis may affect up, but then setting up can affect axis afterwards.
        # vector attributes with interactions
    ...
    # override defaults for scalar attributes with side effects
        ...
        for a in attrs:
            if a in args:
                # use setter to take care of side effects
                setattr(self, a, args[a])
                if scalarInteractions[a] not in argsToSend:
                    # e.g. if a is radius, send size
                    argsToSend.append(scalarInteractions[a])
                    # The following makes sure that size.x is nonzero.
                    # It will be reset when axis is no longer zero.
                    if defaultSize is not None and self._size._x == 0:
                        self._size._x = 1
                del args[a]
    ...
    # now put all args to send into cmd
        # size will be computed by the constuctor
        nosize = ['compound', 'extrusion', 'text']
        for a in argsToSend:
            aval = getattr(self, a)
            if isinstance(aval, vector):
                ...
                aval = aval.idx
            if objName in nosize and a == 'size':
                continue  # do not send superfluous size
            cmd[a] = aval
    ...
    # set canvas
        ...
        if _special_clone is not None:
            cmd["_cloneid"] = _special_clone
        self.appendcmd(cmd)
    ...
    # The vector class calls a change function when there's a change in x, y, or z.
        noSize = ['points', 'label', 'vertex',
                  'triangle', 'quad', 'attach_trail']
    ...
    # Ensure that if axis or up is <0,0,0> in constructor, we'll recover eventually:
        if self._axis.mag2 == 0:
            self._save_oldaxis = vector(1, 0, 0)
        if self._up.mag2 == 0:
            self._save_oldup = vector(0, 1, 0)
    ...
    @up.setter
    def up(self, value):
        # this sets self._axis and self._up
        self._save_oldup = adjust_axis(
            self._up, value, self._axis, self._save_oldup)
        if not self._constructing:
            self.addattr('up')
    ...
    def axis(self, value):  # sphere or ring or text or compound have no axis/size link
        if value.mag2 == 0:
            if self._save_oldaxis is None:
                self._save_oldaxis = self._axis
            self._axis = value
        else:
            # this sets self._axis and self._up
            self._save_oldaxis = adjust_up(
                self._axis, value, self._up, self._save_oldaxis)
        if not self._constructing:
            self.addattr('axis')
        if self._sizing:
    ...
    def color(self, value):
        # may be a list of extrusion colors of the form [ [1,0,0], [0,1,0] ]
    ...
    def trail_type(self, value):
        if (value not in ['curve', 'points']):
            raise Exception(
                "ArgumentError: trail_type must be 'curve' or 'points'")
    ...
    def rotate(self, angle=None, axis=None, origin=None):
        ...
        if angle is None:
            raise TypeError(
                'You must specify an angle through which to rotate')
        if axis is None:
            rotaxis = self.axis
        else:
            rotaxis = axis
    ...
    def bounding_box(self):
        centered = ['box', 'compound', 'ellipsoid',
                    'sphere', 'simple_sphere', 'ring']
        x = norm(self._axis)
        y = norm(self._up)
        z = norm(cross(x, y))
        L = self._size.x
        H = self._size.y
        W = self._size.z
        # make a copy of pos, so changes to p won't affect the object
        p = vector(self._pos)
    ...
    # the vector class calls this when there's a change in x, y, or z
    def _on_size_change(self):
        # update axis length when box.size.x is changed
        self._axis.value = self._axis.norm() * self._size.x
        self.addattr('size')
    ...
    # don't print something when making an (anonymous) object
...
class sphere(standardAttributes):
    ...
    def size(self, value):
        ...
        self._size = value
        if not self._constructing:
            # changing a sphere size should not affect axis
            self.addattr('size')
...
class arrow(standardAttributes):
    ...
    def round(self, value):
        raise AttributeError(
            'Cannot change the "round" attribute of an arrow.')
...
def attach_arrow(o, attr, **args):  # factory function returns arrow with special attributes
    ...
    if not isinstance(getattr(o, attr), vector):
        raise AttributeError(
            'The attach_arrow attribute "'+attr + '" is not a vector.')
    if not isinstance(o.pos, vector):
        raise AttributeError("The object's pos attribute is not a vector.")

    scale = 1
    if 'scale' in args:
        scale = args['scale']
    shaftwidth = 0.5*o._size.y
    if 'shaftwidth' in args:
        shaftwidth = args['shaftwidth']
    c = o.color
    if 'color' in args:
        c = args['color']
...
def attach_light(o, **args):  # factory function returns local_light with special attributes
    ...
    if 'color' in args:
        if not isinstance(args['color'], vector):
            raise AttributeError("The color attribute must be a vector.")
    else:
        args['color'] = o.color  # default color
    if 'offset' in args:
        if not isinstance(args['offset'], vector):
            raise AttributeError(
                'The attach_light attribute "offset" must be a vector.')
    else:
        args['offset'] = vector(0, 0, 0)  # default offset
    a = local_light(pos=o.pos+args['offset'], attach_idx=o.idx,
                    color=args['color'], offset=args['offset'])
    return a
...
class compound(standardAttributes):
    ...
    def __init__(self, objList, **args):
        ...
        if '_cloneid' in args:
            ...
            for obj in objList:
                if obj.canvas is not cvs:
                    raise AttributeError(
                        'all objects used in compound must belong to the same canvas')
                if type(obj) in ineligible:
                    raise TypeError('A ' + obj._objName +
                                    ' object cannot be used in a compound')
                idxlist.append(obj.idx)
            args['obj_idxs'] = idxlist
        # to keep standardAttributes happy
        args['_default_size'] = vector(1, 1, 1)
        savesize = None
        if 'size' in args:
            savesize = args['size']
        ...
        # no axis/size connection except that changing axis.mag changes compound length
        self._sizing = False
...
# no setter; must be set in constructor; this is done in standardAttributes
    ...
    def origin(self, value):  # compound origin cannot be reset
        if not self._constructing:
            raise AttributeError(
                'The compound "origin" attribute is read-only; change "pos" instead.')
        self._origin = value
...
class vertex(standardAttributes):
    ...
    def triangleCount(self, val):
        raise AttributeError(
            'use decrementTriangleCount or incrementTriangleCount')
    ...
    def texpos(self, value):
        if not isinstance(value, vector):
            raise AttributeError(
                'texpos must be a 3D vector with a zero z component')
        if value.z != 0:
            raise AttributeError(
                'the z component of texpos must currently be zero')
        self._texpos = value
        if not self._constructing:
            self.addattr('texpos')
...
class triangle(standardAttributes):
    ...
    def vs(self, value):
        if not isinstance(value, list) or len(value) != 3:
            raise AttributeError(
                'A triangle must be a list of 3 vertex objects.')
        for i in range(3):
...
class curveMethods(standardAttributes):
    ...
    def process_args(self, *args1, **args):
        ...
        if len(tpos) == 0:
            raise AttributeError(
                "To add a point to a curve or points object, specify pos information.")
        pts = []
        cps = []
        for pt in tpos:
            col = c
    ...
    # return a list of dictionaries of the form {pos:vec, color:vec ....}
    def parse_pos(self, *vars):
        ...
        for v in vars:
            if isinstance(v, vector) or isinstance(v, list) or isinstance(v, tuple):
                # legal in GlowScript: pos=[(x,y,z), (x,y,z)] and pos=[[x,y,z], [x,y,z]]
                if not isinstance(v, vector):
                    v = list_to_vec(v)
                if not self._constructing:
                    # make a copy of the vector; it could be (and often is, e.g. in a trail) object.pos
                    ret.append({'pos': vector(v)})
                else:
                    ret.append(vector(v))
                ...
                if not self._constructing:
                    raise AttributeError(
                        "Point information must be a vector or a dictionary")
                else:
    ...
    def point(self, N):
        if N >= len(self._pts) or (N < 0 and -N > len(self._pts)):
            raise ValueError(
                'N = {} is outside the bounds 0-{} of the curve points'.format(N, len(self._pts)-1))
        info = self._pts[N]
        if 'color' not in info:
            info['color'] = self.color
        if 'radius' not in info:
            info['radius'] = self.radius
        if 'visible' not in info:
            info['visible'] = self.visible
        if 'retain' not in info:
            info['retain'] = self.retain
        return info
    ...
    # args1 could be p1, p2, p3 or [p1, p2, p3]
    def splice(self, start, howmany, *args1):
        if howmany < 0:
            raise ValueError(
                'You cannot delete a negative number of points'.format(howmany))
        if start >= len(self._pts) or (start < 0 and -start >= len(self._pts)):
            raise ValueError(
                'The starting location, {}, is outside the bounds of the list of points'.format(start))
        if start >= 0:
            if start+howmany > len(self._pts):
                raise ValueError(
                    'The starting position plus deletions is beyond the list of points'.format(howmany))
        else:
            if start+howmany >= 0:
                raise ValueError(
                    'The starting position plus deletions is beyond the list of points'.format(howmany))
        ...
        self.appendcmd({"val": [start, howmany, cps[:]],
                       "method": "splice", "idx": self.idx})

    def modify(self, N, *arg1, **args):
        attrs = ['pos', 'color', 'radius', 'visible', 'retain']
        if N >= len(self._pts) or (N < 0 and -N >= len(self._pts)):
            raise ValueError(
                'N = {} is outside the bounds 0-{} of the curve points'.format(N, len(self._pts)))
        p = self._pts[N]
        cp = {}
...
class curve(curveMethods):
    def __init__(self, *args1, **args):
        ...
        self._radius = 0
        # cumulative list of dicts of the form {pos:vec, color=vec, radius=r, visible=T/F} python side
        self._pts = []
        ...
        if tpos is not None:
            ...
            self.append(tpos)
        if len(args1) > 0:
            self.append(*args1, color=self.color,
                        radius=self.radius, visible=self.visible)
...
class gobj(baseObj):
    ...
    # default values of shared attributes
        ...
        if 'data' in args:
            ...
            # call plot to resolve pos arguments into self._plot
            self.plot(datatemp)
            del args['data']
        ...
        for a in argsToSend:
            if a == 'interval':
                continue  # do not send to browser; handle here instead
            aval = getattr(self, a)
            if isinstance(aval, vector):
                aval = aval.value
            cmd[a] = aval
    ...
    def color(self, val):
        if not isinstance(val, vector):
            raise TypeError('color must be a vector')
        self._color = val
        self.addattr('color')
    ...
    def resolveargs(self, *vars):
        ...
        if isinstance(vars[0], list) or isinstance(vars[0], tuple):
            # plot([x,y], [x,y])
            if not isinstance(vars[0][0], list) and not isinstance(vars[0][0], tuple):
                for v in vars:
                    ...
                    if isinstance(v, list) and len(v) == 2:
                        ...
                        raise AttributeError(
                            "Plot data must be 2-dimensional, like [x,y].")
            else:                           # plot([ [x,y], [x,y] ]
                for v in vars[0]:
                    if isinstance(v, tuple):
                        v = list(v)
    ...
    def preresolve1(self, *args):
        if isinstance(args[0], list) or isinstance(args[0], tuple):
            if len(args) == 1 and len(args[0]) == 1:
                return self.resolveargs(args[0][0])
            a = []
            for arg in args[0]:
                a.append(arg)
    ...
    def preresolve2(self, args):
        if 'color' in args:
            raise AttributeError(
                "Cannot currently change color in a plot statement.")
        if 'pos' in args:
    ...
    # don't print something when making an (anonymous) graph object
    def _ipython_display_(self):
        pass
...
class gcurve(gobj):
    ...
    def marker_color(self, val):
        if not isinstance(val, vector):
            raise TypeError('marker_color must be a vector')
    ...
    def dot_color(self, val):
        if not isinstance(val, vector):
            raise TypeError('dot_color must be a vector')
...
class graph(baseObj):
    ...
    # default values
        ...
        if self._scroll:
            if not ('xmin' in argsToSend and 'xmax' in argsToSend):
                raise AttributeError(
                    "For a scrolling graph, both xmin and xmax must be specified.")
            if self._xmax <= self._xmin:
                raise AttributeError(
                    "For a scrolling graph, xmax must be greater than xmin.")
    ...
    def align(self, val):
        if not (val == 'left' or val == 'right' or val == 'none'):
            raise NameError(
                "align must be 'left', 'right', or 'none' (the default).")
        self._align = val
        self.addattr('align')
    ...
    def foreground(self, val):
        if not isinstance(val, vector):
            raise TypeError('foreground must be a vector')
    ...
    # don't print something when making an (anonymous) graph
    def _ipython_display_(self):
        pass
...
class faces(object):
    def __init__(self, **args):
        raise NameError(
            'faces is no longer supported; use vertex with triangle or quad')
...
class Camera(object):
    ...
    def axis(self, value):
        c = self._canvas
        c.autoscale = False
        # use current self.pos before it is changed by change in c.axis
        c.center = self.pos+value
        c.axis = norm(value)
    ...
    def rotate(self, angle=0, axis=None, origin=None):
        ...
        if origin is not None and origin != self.pos:
            origin = self.pos + \
                (self.pos-origin).rotate(angle=angle, axis=axis)
        else:
            origin = self.pos
...
class canvas(baseObj):
    ...
    def __init__(self, **args):
        ...
        if 'lights' in args:
            raise AttributeError(
                "Lights for a canvas can be assigned only after the canvas has been created.")
        ...
        # self.axis is primal internally; self._forward is now a synonym
        self._forward = vector(0, 0, -1)
        # user with touch screen can rotate around z
        self._up = vector(0, 1, 0)
    ...
    # send only nondefault values to GlowScript
        ...
        if "visualize" in args:  # [UCR]
            baseObj.visualize = args["visualize"]  # [UCR]

        if "catsoop_ports" in args:  # [UCR]
            baseObj.visualize = False  # [UCR]
            baseObj.catsoop = True  # [UCR]
            baseObj.catsoop_ports = args["catsoop_ports"]  # [UCR]
    ...
    def follow(self, obj):
        if obj is None:
            ...
            b = box(visible=False)
            baseObj.follow_objects.append(
                [b, obj, vector(1.2e15, 3.4e14, -5.6e13)])
    ...
    def lights(self, value):
        if type(value) is list and len(value) == 0:
            ...
            self._lights = []
            # don't encode this unusual statement
            self.appendcmd({"lights": 'empty_list'})
        else:
...
# key events conflict with notebook command mode; not permitted for now
        ...
        if ev == 'pick':
            ...
            if obj._objName == 'text':
                ...
                if obj._objName == 'extrusion':
                    # use the first segment color to represent multicolor extrusions
                    obj._color = obj._firstcolor
            ...
            # user can change forward (spin), range/autoscale (zoom), up (touch), center (pan)
            else:
                if 'forward' in evt and self.userspin and not self._set_forward:
                    fwd = evt['forward']
                    # the fundamental meaning of scene.forward is scene.axis
                    self._axis = list_to_vec(fwd)
                self._set_forward = False
    ...
    # don't print something when making an (anonymous) canvas
    def _ipython_display_(self):
        pass
    ...
    def setup(self, args):
        ...
        if 'pos' in args:
            ...
            if self.location == print_anchor:
                # self.location = [-1, print_anchor]
                raise AttributeError(
                    self.objName+': Cannot specify "print_anchor" in VPython 7.')
            argsToSend.append('location')
            del args['pos']
...
class radio(controls):
    ...
    def name(self, value):
        if not self._constructing:
            raise AttributeError(
                'Cannot change the name attribute of a radio widget.')
...
class slider(controls):
    ...
    def vertical(self, value):
        raise AttributeError(
            'vertical cannot be changed after creating a slider')
    ...
    def length(self, value):
        raise AttributeError(
            'length cannot be changed after creating a slider')
    ...
    def bottom(self, value):
        raise AttributeError(
            'bottom cannot be changed after creating a slider')
...
class extrusion(standardAttributes):
    def __init__(self, **args):
        ...
        for attr in ['scale', 'xscale', 'yscale', 'twist']:
            if attr in args and isinstance(args[attr], list) and len(args[attr]) != self._pathlength:
                raise AttributeError(
                    "The "+attr+" list must be the same length as the list of points on the path ("+str(self._pathlength)+").")
    ...
    def level(self, a):
        ...
        #                      [ [S, S, ... ], [S, S, ...], [S, S, ...] ], ... ]
        if not (type(a) is list or type(a) is tuple):
            raise AttributeError("A shape must be a list of [x,y] elements.")
        if not (type(a[0]) is list or type(a[0]) is tuple):
            raise AttributeError("A shape must be a list of [x,y] elements.")
        if type(a[0][0]) is int or type(a[0][0]) is float:
            return 1
        if type(a[0][0][0]) is int or type(a[0][0][0]) is float:
            return 2
        if type(a[0][0][0][0]) is int or type(a[0][0][0][0]) is float:
            return 3
        raise AttributeError(
            "A shape must be a list of lists of [x,y] elements.")
        ...
        if n == 1:
            ...
            for c in a:
                for b in c:
                    ...
                    if not (p1[0] == p2[0] and p1[1] == p2[1]):
                        raise AttributeError(
                            "An extrusion shape must be closed.")
    ...
    def path(self, value):
        raise AttributeError(
            'path cannot be changed after extrusion is created')
    ...
    def shape(self, value):
        raise AttributeError(
            'shape cannot be changed after extrusion is created')
    ...
    def smooth(self, value):
        raise AttributeError(
            'smooth cannot be changed after extrusion is created')

    def checkjointlist(self, js, name):
        if not isinstance(js, list):
            raise AttributeError(name+' must be a list of joint indices.')
        for k in js:
            if k < 0 or k > self._pathlength:
                raise AttributeError(str(
                    k)+' is not in the range of joints in the path (0-'+str(self._pathlength-1)+').')
    ...
    def smooth_joints(self, value):
        raise AttributeError(
            'smooth_joints cannot be changed after extrusion is created')
    ...
    def sharp_joints(self, value):
        raise AttributeError(
            'sharp_joints cannot be changed after extrusion is created')
    ...
    def show_start_face(self, value):
        raise AttributeError(
            'show_start_face cannot be changed after extrusion is created')
    ...
    def show_end_face(self, value):
        raise AttributeError(
            'show_end_face cannot be changed after extrusion is created')
    ...
    def start_face_color(self, value):
        raise AttributeError(
            'start_face_color cannot be changed after extrusion is created')
    ...
    def end_face_color(self, value):
        raise AttributeError(
            'end_face_color cannot be changed after extrusion is created')
    ...
    def start_normal(self, value):
        raise AttributeError(
            'start_normal cannot be changed after extrusion is created')
    ...
    def end_normal(self, value):
        raise AttributeError(
            'end_normal cannot be changed after extrusion is created')
    ...
    def twist(self, value):
        raise AttributeError(
            'twist cannot be changed after extrusion is created')
    ...
    def scale(self, value):
        raise AttributeError(
            'scale cannot be changed after extrusion is created')
    ...
    def xscale(self, value):
        raise AttributeError(
            'xscale cannot be changed after extrusion is created')
...
class text(standardAttributes):
    ...
    def __init__(self, **args):
        ...
        if '_cloneid' in args:
            ...
            if 'text' in args:
                ...
                raise AttributeError(
                    'A text object must have a text attribute')
        if 'color' in args:
    ...
    def axis(self, value):  # changing axis does not affect size
        ...
        self._axis.value = value
        self._save_oldaxis = adjust_up(
            norm(old), self._axis, self._up, self._save_oldaxis)
        self.addattr('axis')
        self.addattr('up')
    ...
    def length(self, val):
        if self._constructing:
            raise AttributeError(
                "text length can't be set when creating a text object")
        else:
            self._length = val
            self.addattr('length')
...
def combin(x, y):
    ...
    if nn != x+1:
        raise ValueError(
            'Illegal arguments (%d, %d) for combin function' % (x, y))
    return num
