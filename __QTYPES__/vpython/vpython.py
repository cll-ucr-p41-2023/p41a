...
defaults["csq_vpython"] = True
defaults["csq_vpython_sim"] = ""
defaults["csq_vpython_with_graph"] = False
...
def handle_check(submissions, **info):
    ...
    if info["csq_vpython"]:
        code_copy = code
        code_pre_copy = info["csq_code_pre"]

        info["csq_code_pre"] = "\n\n".join([f"from vpython import *\nscene = canvas(visualize=False)",
                                            info["csq_code_pre"]])
        info["csq_sandbox_options"]["CLOCKTIME"] = 60

    code = "\n\n".join(["import os\nos.unlink(__file__)",
                       info["csq_code_pre"], code, info["csq_code_post"]])
    ...
    if not timeout:
        ...
        if complete:
            ...
            if info["csq_vpython"]:
                ...
                __HTTP_PORT = find_free_socket()
                __SOCKET_PORT = find_free_socket()

                def run_vpython_sim():
                    sim_code = "\n\n".join(
                        [f"import os\nos.unlink(__file__)\nfrom vpython import *\nscene = canvas(catsoop_ports=[{__HTTP_PORT}, {__SOCKET_PORT}])", code_pre_copy, code_copy, info["csq_vpython_sim"]])
                    info["sandbox_run_code"](
                        info, sim_code, info.get("csq_sandbox_options", {}))

                sandbox_thread = threading.Thread(
                    target=run_vpython_sim)
                sandbox_thread.start()

                time.sleep(1)
                msg += f'''
                <div id="{info["csq_name"]}_vpython" class="vpython_simulation">
                    <div id="{info["csq_name"]}_timer"></div>
                    <div id="{info["csq_name"]}_simulation">
                        <iframe id="{info["csq_name"]}_iframe" src="http://localhost:{__HTTP_PORT}/" width="100%" height="{820 if info["csq_vpython_with_graph"] else 410}">
                </iframe></div></div>
                '''
        else:
...
def handle_submission(submissions, **info):
    ...
    if info["csq_vpython"]:
        info["csq_code_pre"] = "\n\n".join([f"from vpython import *\nscene = canvas(visualize=False)", info["csq_code_pre"]])
